...
class SlidingWindowSearch:
    ...
    def __init__(self):
        self.dt = DetectionTraining()
        self.tp = TrainingParameters()
        self.sp = WindowSearchParameters()
    ...
    def single_img_features(self, img, color_space='RGB', spatial_size=(32, 32),
        ...
        if spatial_feat == True:
            ...
            spatial_features = self.dt.bin_spatial(feature_image, size=spatial_size)
            # 4) Append features to list
            img_features.append(spatial_features)
        # 5) Compute histogram features if flag is set
        if hist_feat == True:
            hist_features = self.dt.color_hist(feature_image, nbins=hist_bins)
        ...
        if hog_feat == True:
            if hog_channel == 'ALL':
                ...
                for channel in range(feature_image.shape[2]):
                    hog_features.extend(self.dt.get_hog_features(feature_image[:, :, channel],
                                                         orient, pix_per_cell, cell_per_block,
                                                         vis=False, feature_vec=True))
            else:
                hog_features = self.dt.get_hog_features(feature_image[:, :, hog_channel], orient,
